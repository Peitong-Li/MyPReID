...
@time: 2021/4/20 18:48
@desc: Manager PReid datasets
...
class Market1501(object):
    ...
    def __init__(self, root='data', **kwargs):
        ...
        # extract data from dir: for train dataset, need to relabel and others without
        train, num_train_pids, num_train_imgs = self._process_dir(self.train_dir, relabel=True)
        query, num_query_pids, num_query_imgs = self._process_dir(self.query_dir, relabel=False)
        gallery, num_gallery_pids, num_gallery_imgs = self._process_dir(self.gallery_dir, relabel=False)
        num_total_pids = num_train_pids + num_query_pids   # the number of person for the dataset
        num_total_imgs = num_train_imgs + num_query_imgs + num_gallery_imgs  # the number of images for the dataset

        # print info of extracted data
        ...
        # make the data become our class attribute
        self.train = train
        self.query = query
    ...
    def _process_dir(self, dir_path, relabel=False):
        """
        desc: process datasets, get pid,cid
        :param dir_path: dataset path, eg. train,query,gallery
        :param relabel: if relabel for the dataset
        :return:
        1. dataset: a tuple with image path, person id and camera id
        2. num_pids: the number of person id ,
        3. num_imgs: the number of dataset images what you assigned
        """
...
class DukeMTMCreID(object):
    ...
    def _check_before_run(self):
        """Check file if all files are available before going deeper"""
        if not osp.exists(self.dataset_dir):
            raise RuntimeError("'{}' is not available".format(self.dataset_dir))
        if not osp.exists(self.train_dir):
            raise RuntimeError("'{}' is not available".format(self.train_dir))
    ...
    def _process_dir(self, imgs_dir, relabel=False):
        """
        desc: process datasets, get pid,cid
        :param dir_path: dataset path, eg. train,query,gallery
        :param relabel: if relabel for the dataset
        ...
        3. num_imgs: the number of dataset images what you assigned
        """
        imgs = glob.glob(osp.join(imgs_dir, '*.jpg'))
        pattern = re.compile(r'([-\d]+)_c(\d)')
...
class CUHK03(object):
    ...
    def __init__(self, root='data', split_id=0, cuhk03_labeled=False, cuhk03_classic_split=False, **kwargs):
        ...
        self.data_mat_path = osp.join(self.data_dir, 'cuhk-03.mat')

        # two type dataset：detected（method labeled）, labeled（handcraft labeled）
    ...
    def _check_before_run(self):
        ...
        if not osp.exists(self.data_mat_path):
            raise RuntimeError("'{}' is not available".format(self.data_mat_path))
    ...
    def _preprocess(self):
        ...
        if osp.exists(self.imgs_detected_dir) and \
            osp.exists(self.imgs_labeled_dir) and \
        ...
        print("Extract image data from {} and save as png".format(self.data_mat_path))
        mat = h5py.File(self.data_mat_path, 'r')
...
def get_names():
    return list(__image_factory.keys()) + list(__vedio_factory)

def init_img_dataset(name, **kwargs):
    """Build data object"""
    if name not in __image_factory.keys():
        raise KeyError("Invalid dataset, got '{}', but expected to be one of {}".format(name, __image_factory.keys()))
    return __image_factory[name](**kwargs)


# if __name__ == '__main__':
#     dataset = init_img_dataset('chuk03', root=r'E:\WorkSpace\Datasets')